...
type RelevancePlugin interface {
    Plugin
    // IsRelevantForPodResize returns a Status indicating whether this plugin is relevant
    // for evaluating the given resizing pod. If it returns fwk.Skip, the framework skips all
    // extension points for this plugin during the scheduling cycle and preemption.
    IsRelevantForPodResize(ctx context.Context, cycleState *CycleState, pod *v1.Pod) *fwk.Status
}
```

At the beginning of a scheduling cycle for a resizing pod, `IsRelevantForPodResize` is evaluated so
...
`RelevancePlugin` to return success for resizing pods.
...
ignore deferred pods, as they will only be invoked for resizing pods if they
...
`IsRelevantForPodResize(ctx, cycleState, pod)`.
...
The scheduler logic will be covered by regular feature tests, which is sufficient because
preemption code changes occur purely in-memory.

For the newly introduced API, we added dedicated tests for feature gate enablement/disablement
in the API server:
- https://github.com/kubernetes/kubernetes/blob/9f663aa01a8c3587ec1d2547dd2b555763511c1f/pkg/registry/core/node/strategy_test.go#L538
- https://github.com/kubernetes/kubernetes/blob/9f663aa01a8c3587ec1d2547dd2b555763511c1f/pkg/registry/core/node/strategy_test.go#L538
